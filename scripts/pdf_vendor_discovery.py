...
from downloads_organizer import DEFAULT_ROOT, where_froms
from pdf_text import extract_pdf_text
from vendor_rules import all_vendor_rules, match_vendor
...
def canonical_entity(entity, vendor_rules):
    known = match_vendor(entity, vendor_rules)
    if known != "Unknown":
...
def extract_entities(text, vendor_rules):
    ...
    for idx, line in enumerate(lines[:1200]):
        ...
        for start, end, _suffix in suffix_spans(line):
            ...
            if not raw_entity:
                continue
            display, key, known = canonical_entity(raw_entity, vendor_rules)
            if len(key) < 3:
                continue
...
def brand_tokens(entity):
    ...
    return [t for t in tokens if len(t) >= 3 and t not in LEGAL_WORDS and t not in BRAND_STOPWORDS]
...
def iter_pdfs(root, scope):
    if scope == "managed":
        managed = root / "_PDF"
        if not managed.is_dir():
            raise SystemExit(f"Managed PDF folder not found: {managed}")
        return sorted(p for p in managed.rglob("*.pdf") if p.is_file() and not p.name.startswith(".")), managed

    return sorted(p for p in root.iterdir() if p.is_file() and p.suffix.lower() == ".pdf"), root


def main():
    ap = argparse.ArgumentParser(description="Discover recurring PDF vendors locally.")
    ap.add_argument("--root", default=str(DEFAULT_ROOT), help="Downloads root")
    ap.add_argument("--scope", choices=["inbox", "managed"], default="inbox", help="inbox=top-level Downloads; managed=recursive _PDF")
    ...
    pdfs, scan_root = iter_pdfs(root, ns.scope)
    ...
    for path in pdfs:
        ...
        known = match_vendor(haystack, vendor_rules)
        if known != "Unknown":
            known_counts[known] += 1

        hosts = source_hosts(origins)
        doc_id = str(path.relative_to(root))
        for display, key, role, _context, raw_entity, known_vendor, line_idx in extract_entities(text, vendor_rules):
            ...
            if line_idx < 25:
                entity_header_docs[key].add(doc_id)
            if source_matches_entity(display, hosts):
                entity_source_docs[key].add(doc_id)
            for host in hosts:
                entity_hosts[key][host] += 1
            if len(entity_files[key]) < 20:
                entity_files[key].append(doc_id)

    suffix = "_managed" if ns.scope == "managed" else ""
    summary_csv = report_dir / f"pdf_vendor_candidates{suffix}.csv"
    detail_csv = report_dir / f"pdf_vendor_candidate_details{suffix}.csv"
    rules_path = report_dir / f"pdf_vendor_rule_suggestions{suffix}.py"
    ...
    if mentions:
        if ns.show_mentions:
            ...
            for r in mentions[:80]:
                print(f"  {r['documents']:4} docs  header={r['header_hits']:>3}  buyer={r['buyer_hits']:>3}  mention  {r['candidate']}")
        else:
    ...
    if ns.emit_rules:
        ...
        with rules_path.open("w", encoding="utf-8") as f:
            f.write("# AUTO-GENERATED LOCAL DRAFT. REVIEW BEFORE COPYING INTO PRIVATE RULES.\n")
    ...
    if ns.scope == "managed":
        print("Only _PDF was traversed recursively; no other Downloads subdirectory was touched.")
    else:
